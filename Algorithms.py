...
def getGroupstreatments(DAG, df, groupingAtt, groups, ordinal_atts, targetClass,
    ...
    # pool = Pool(processes=CPU_COUNT)  # Change the number of processes as per your requirements
    #
...
def findSolGreedy(groups_dic, k, tau, high, low):
    sol = []
    k_new = min(k, len(groups_dic))
    for group in groups_dic:
        if high and low:
            sol.append((group, groups_dic[group]['epxlainability'], groups_dic[group]['treatment_low'],
                        groups_dic[group]['cate_low'], groups_dic[group]['treatment_high'],
                        groups_dic[group]['cate_high']))
        if low:
            sol.append((group, groups_dic[group]['epxlainability'], groups_dic[group]['treatment_low']))
        else:
            sol.append((group, groups_dic[group]['epxlainability'], groups_dic[group]['treatment_high']))

    ans = sorted(sol, key=lambda x: x[1], reverse=True)[0:k_new]
    exp = [g[1] for g in ans]
    exp = sum(exp)
    coverage = set()

    # for g in ans:
    #     covered = groups_dic[g[0]]['covered']
    #     for cov in covered:
    #         coverage.add(cov)
    return ans, exp, len(coverage), coverage